...
Como se explicó en la sección \ref{FabClavos}, se encontró que el método más efectivo para refinar el tamaño de grano es la utilización de $B$ como refinador. El boro reacciona con el aluminio de la aleación formando $AlB_2$ que es el compuesto responsable por el efecto refinador. 

Este compuesto fue utilizado por \cite{ritelst} quien obtuvo para unas concentraciones de $con1$ y $conc2$ un tamaño de grano medio de $tam 1$ y $tam 2$ respectivamente.
...
Respecto del método de fabricación, se comprobó que las concentraciones de refinador utilizadas en el trabajo de \cite{ritelst} son efectivas para obtener una disminución importante del tamaño de grano. Queda demostrada que la incorporación del refinador no es sencilla. \cite{ritelst} muestra los tamaños de grano que obtuvo, pero no indica si el polvo refinador fue incorporado totalmente o si se encontró con la misma dificultad. Además se concluyó que la molienda mecánica de la aleación con el refinador es efectiva para la incorporación, aunque no así el prensado de pastillas antes de realizar la fundición. También se demostró el efecto refinador observado por la presencia de óxido formado en las distintas etapas del proceso. Este óxido es prácticamente imposible de evitar a menos que se trabaje con atmósfera de argón desde los primeros cortes de la aleación, la molienda y posterior fundición. También queda por resolver la porosidad encontrada al fundir el polvo suelto en el tubo de cuarzo. Seguramente se necesitará alguna compactación para evitar esta porosidad, pero no tan compacto como las pastillas que se fabricaron.

Aumento del tamaño de grano

Respecto de las características de las aleaciones obtenidas, el \textbf{Clavo 5},que presenta el mayor tamaño de grano, es el que posee las mejores propiedades de memoria de forma. Luego de cada ciclo de deformación, el porcentaje de deformación recuperado es mayor que en los otros clavos. Además las tensiones de transformación son menores. Este resultado era esperado, ya que tener un mayor tamaño de grano permite movimiento más libre de las interfases entre las placas de martensita y la austenita, ya que es una estructura con menor cantidad de bordes de grano y menos barreras como el polvo refinador y las partículas de óxido.

A partir de los resultados obtenidos se concluyó que el método óptimo para refinar el tamaño de grano es la utilización de una pequeña cantidad de $AlB_2$ como refinador ($0.005 \%$ pp es suficiente), el cual debe ser incorporado a traves de una molienda mecánica. Si bien se obtuvo un tamaño de grano similar al utilizar únicamente la molienda mecánica de la aleación sin el agregado de polvo refinador, se recomienda utilizar una pequeña fracción de $AlB_2$ ya que el proceso de fabricación de las esponjas requerirá varias fundiciones, y la utilización de una pequeña cantidad de refinador ayudará a garantizar que no se produzca un aumento del tamaño de grano con cada aumento de la temperatura.
...
Un posible método para analizar la integridad de una esponja es por microscopía o tomografía, que permiten ver la superficie y el interior de la estructura. Pero sería de gran utilidad poder monitorear la integridad mientras el dispositivo se encuentra en servicio. Por otro lado, la cantidad de variables asociadas a la deformación de las esponjas y transformación de la aleación es muy amplia, lo que hace que la transformación de estas estructuras sea un proceso complejo de entender. Además de poder entender su comportamiento, el gran potencial deaplicaciones, motivó la búsqueda de algún método del seguimiento de los procesos que ocurren y la integridad estructural de la esponja.

Una forma de caracterizar la estructura de una esponja, y poder comprobar el estado de rotura de la misma es por medio de microscopía o tomografía. Pero son métodos que no pueden realizarse in situ, sin sacar de funcionamiento el dispositivo, y requieren el uso de equipos complejos y costosos. Un método muy conocido de seguimiento de las transformaciones martensíticas es la medición de la resistencia eléctrica. Este método solo requiere el uso de cuatro cables soldados a la muestra, se mide la resistencia con el método de las cuatro puntas y solo se necesita un equipo que pueda medir la resistencia con buena precisión. 

A continuación se presentan las mediciones realizadas y el análisis de los datos obtenidos. A partir de estos se buscó obtener parámetros que brinden información sobre los procesos que ocurren y la integridad estructural de la esponja. Se inició el estudio analizando un monocristal de CuZnAl ya que es la estructura mas sencilla que se encuentra en un material. Luego se realizó el mismo análisis con un policristal y finalmente se aplicaron los resultados obtenidos a una esponja de CuZnAl.
...
Luego, si la probeta se comprimió un porcentaje $\varepsilon$, considerando que el volumen se mantendrá constante, las dimensiones de la probeta serán 
...
Por lo tanto, 
\begin{equation}
R= \rho \frac{l}{A} = \rho (1-\frac{\varepsilon}{100})^2 \frac{l_0}{A_0} %= \rho f^2 \frac{l_0}{A_0}
\end{equation}

De esta manera, $(1-\frac{\varepsilon}{100})^2$ es un factor que relaciona el cambio de la resistencia con los cambios geométricos de la muestra. 

%TODO Hacer grafico de esta HistExplicacion

A continuación agregaremos los cambios de resistencia debidos a la transformación martensítica. Como muestra la figura \textbf{--falta hacer el esquema de este gráfico--}, si inicialmente la muestra se encuentra en estado austenítico, tendrá una resistencia $R_A$. Al disminuir la temperatura, transformará a martensita y la resistencia aumentará al valor $R_0$. Pero no se está tomando en cuenta la compresión que sufre la muestra al transformar, que generará una disminución en la resistencia ,que entonces llegará al valor $R$. Por lo tanto tendremos:
...
Se comenzó estudiando un monocristal de CuZnAl que fue fabricado anteriormente por el grupo de investigación. La muestra, que tenía una forma inicial cilíndrica, se preparó siguiendo el procedimiento explicado en la sección \ref{compresión}. Antes de comenzar el ensayo se realizó un recocido a $800$ $ ^\circ C$ durante media hora y luego un enfriamiento al aire para reiniciar el estado termomecánico del material. A continuación se soldaron los cuatro cables utilizados para medir la resistencia del material por el método de las cuatro puntas y una termocupla para registrar la temperatura de la muestra durante todo el proceso. Seguidamente se realizaron ciclos de transformación térmica de la muestra, variando la temperatura de la cámara ambiental, y con un extensómetro se registró la dilatación/contracción de la muestra en cada ciclo de transformación. Cada uno de los ciclos de transformación se realizó manteniendo distintas cargas constantes. 

La figura \ref{resultados6b} muestra los gráficos obtenidos para una de las muestras ensayadas. En la figura \ref{fig: strain6b} se graficó la curva tensión vs deformación, donde inicialmente se aumentó la carga manualmente hasta el valor correspondiente al ensayo, en la figura $32.7$ $MPa$, y luego se mantuvo constante manualmente durante el ciclo térmico de transformación. La tensión se mantuvo constante variando la velocidad y dirección de carga de la jaula de compresión de la máquina de ensayos mecánicos. Al enfriar, la muestra transformaó a variantes de martensita que relajando la tensión a la cual estaba sometida la muestra (variantes compresivas). Como puede verse en el gráfico \ref{fig: strain6b} la muestra redujo su longitud aproximadamente un $2.6\%$. Al aumentar la temperatura transformó a austenita, la longitud volvió al valor inicial, y finalmente la carga se retiró manualmente. En la figura \ref{fig: res6b} se presenta la histéresis que se obtuvo al graficar la resistencia de la muestra en el ciclo de transformación. 

% \vspace{5mm} %5mm vertical space
% \textbf{$*****$No estoy segura si estas descripciones deberían ir en la sección de procedimientos...$*****$}
% \vspace{5mm} %5mm vertical space

 En la figura \ref{EjStrainRes} se muestran los gráficos obtenidos al realizar un ciclo térmico de transformación libre de carga. En la figura \ref{fig: EjStrain}  se presenta la variación de deformación en función de la temperatura registrada por el extensómetro y en la figura \ref{fig: EjRes} su correspondiente histéresis de resistividad.
  ...
  \begin{figure}% \label{fig:Clavo1}
    ...
    ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
      %(or a blank line to force the subfigure onto a new line)
    \begin{subfigure}{0.47\textwidth}
        \includegraphics[width=\textwidth]{Img/Resultados/Resistencia/ejstrain.eps}
        \caption{Corte transversal del Clavo 1}%Corte transversal del Clavo 1}
        \label{fig: EjStrain}
    \end{subfigure}
    \begin{subfigure}{0.47\textwidth}
        \includegraphics[width=\textwidth]{Img/Resultados/Resistencia/ejres.eps}
        \caption{Corte transversal del Clavo 2}%Corte transversal del Clavo 1}
        \label{fig: EjRes}
    \end{subfigure}
    \caption{Gráficos obtenidos de un ciclo térmico de transformación de una probeta policristalina libre de cargas aplicadas. En (a) la curva correspondiente a la deformación en función de la temperatura y en (b) su correspondiente curva de resistencia en funcion de la temperatura.}
    \label{EjStrainRes}
    \end{figure}


Para eliminar la variación por temperatura de la resistividad en estado austenítico y martensítico, deben medirse a la misma temperatura. Como no es posible tener $100 \%$ de ambas fases simultáneamente, los valores de resistencia se tomaron como se muestra en la figura \ref{fig:ExpRes}. Además la figura \ref{fig:ExpStrain} muestra como se tomó el valor de deformación máxima en la transformación.
...
En la tabla \ref{tab:TablaMonocristal} se detallan los valores obtenidos al realizar distintos ensayos con un monocristal y los resultados obtenidos al aplicar la ecuación \ref{fprima}.
...
Tensión ($MPa$) & $\varepsilon$ ($\%$) &  $f'=\frac{R_M}{R_A}$ & $f$  \\ \midrule
 0        &  0.76   & 1.27 & 1.30  \\
 1.93       &  0.37   & 1.28 & 1.30  \\
 17.4      &  -7.60  & 1.15 & 0.98  \\
 31.0      &  -7.90  & 1.17 &  0.99 \\
...
A continuación se aplicó el mismo análisis a las mediciones realizadas con una muestra policristalina. Se efectuaron nuevamente ciclos térmicos de transformación, manteniendo constantes distintas cargas. En la tabla \ref{tab:TablaPolicristal} se muestran las distintas cargas aplicadas, las deformaciones obtenidas, y los valores de $f '$ que relaciona la resistencia en estado martensítico y austenítico. 
% TODO justificacion
...
En el gráfico \ref{fig:PoliMono} se presentan todos los resultados que se obtuvieron al ensayar la muestra monocristalina y una policristalina, correspondientes a las tablas \ref{tab:TablaMonocristal} y \ref{tab:TablaPolicristal}. Puede verse que hay una tendencia lineal en el aumento de la resistencia por las variaciones geométricas, cuya pendiente corresponde al valor de $f$ según la ecuación \ref{fprima}. Se concluyó a partir de este resultado que la variación en el aumento de la resistencia al transformar de austenita a martensita se da por una variación geométrica, y no se observa ningún efecto adicional de la carga aplicada.
...
\subsection{Ensayos realizados con esponjas}
 ...
 En la figura \ref{fig:HistExplicacion} se muestra el gráfico que se obtuvo en un ciclo de transformación térmica de una esponja de CuZnAl al graficar resistencia vs temperatura. Inicialmente se le realizó un recocido a 800 $^\circ C$ y un enfriamiento al aire para asegurar que la muestra se encuentraba en estado austenítico al iniciar el ensayo. Luego se le soldaron los cuatro cables utilizados para el método de las cuatro puntas (seccón \ref{CuatroPuntas}) y una termocupla tipo K para tener un registro preciso de la temperatura del material. Los números en el gráfico indican el recorrido que sigue la resistencia durante el ensayo. 
 
 Inicialmente, en el punto \textit{1}, la muestra se encontraba en estado totalmente austenítico. Se disminuyó la temperatura utilizando una cámara ambiental $numero de la camara$, con una velocidad de $5$ $^\circ C/minuto$. En esta etapa la resistencia varía linealmente con la temperatura.
 ...
 Al alcanzar una temperatura de $-40$ $^\circ C$ se comenzó a aumentar la temperatura nuevamente, a una velocidad de $5$ $^\circ C/minuto$. Ahora la resistividad del material comenzó a aumentar linealmente con la temperatura, hasta llegar a aproximadamente $-10$ $^\circ C$. A esta temperatura ($A_s$), comenzó a transformar a austenita nuevamente, y la resistencia del material disminuyó. Idealmente, la resistencia en estado austenítico debería volver al punto \textit{1}, cerrando la histéresis, pero como se ve en el gráfico vuelve al punto \textit{4}, el cual posee un valor superior al inicial. Esto se debe a dos causas principales, la presencia de austenita retenida y la rotura de la estructura. La austenita retenida se debe a la estabilización de una fracción de martensita que se formó al llegar al punto \textit{2}. Por otro lado la transformación de la estructura genera tensiones entre los granos que transforman, y los granos contiguos que no pueden acomodarse a la deformación de los mismos, por lo que la estructura se rompe formando fisuras intergranulares. En los puntos donde los granos son muy grandes, o las paredes de la estructura son muy finas, este efecto se acentúa. Al romperse distintos puntos de la estructura, disminuye la sección efectiva de la esponja, aumentando así la resistencia.

 Luego de llegar al punto \textit{3} se realizó un nuevo ciclo de enfriamiento. Esta vez, al transformar, la resistencia subió hasta el punto \textit{4}. Si se comparan el punto \textbf{3} y el \textbf{4}, ambos poseen una estructura totalmente martensítica, por lo que podemos concluir que la diferencia que existe en la resistencia entre estas dos curvas se debe a la rotura de la estructura. La diferencia de resistencia entre los segmentos \textbf{2} y \textit{4} es mucho menor a la diferencia entre los segmentos \textit{1} y \textit{3}. Se llegó a la conclusión de que en este primer ciclo, el aumento de resistencia entre los segmentos \textit{1} y \textit{3} se debe a la presencia de martensita retenida, pero también a la formación de fisuras en la estructura de la esponja debido a las tensiones que se generaron en la transformación. 
 
 \vspace{5mm}
 Ahora se aplicará un análisis análogo al de la ecuación \ref{fprima} a los resultados obtenidos de los ensayos realizados con una esponja. Si la transformación de martensita a austenita fuese completa, la ecuación \ref{fprima} también lo estaría. Pero no se toma en cuenta la martensita retenida que estará siempre presente. Luego de realizar un ciclo de transformación a martensita y volver a austenita, la resistencia estará dada por:
\begin{equation}
 R_1=\rho_A \frac{L_0}{A}(1-b) + \rho_M \frac{L_0}{A}b
\end{equation}
Donde ahora $0<b<1$ representa la fracción de martensita retenida y $(1-b)$ la aleación restante que transforma a austenita. En este punto, la resistencia estará dada por la austenita, martensita retenida, y además por la rotura de la estructura (valor $R_1$ en la figura \ref{fig:HistEsquema}). Si $f$ es el cociente entre las resistividades en estado martensítico y austenítico $f=\rho_M / \rho_ A$, entonces:
\begin{equation}
R1= \rho_A \frac{L_0}{A} [(1-b)+fb]=R \frac{A_0}{A}[1+b(f-1)] = R \rho_A \frac{L}{A} (1+\varepsilon)^2 [1-b+bf]
\end{equation}
\begin{equation}
 R = R_A (1+\varepsilon)^2 [1+b(f-1)] \label{austret}
\end{equation}



De esta forma la resistencia medida en estado martensítico $R$ toma en cuenta un factor geométrico  $(1+ \varepsilon)^2$, y un término $b$ correspondiente a la fracción de martensita retenida. Del análisis realizado en la sección \ref{PoilicMonocrist} se encontró que $f=~1.3$ representa una buena aproximación. Además si se realizó un ciclo de transformación completo, y la probeta retransformó a austenita recuperando su forma inicial, entonces el término geométrico $(1+\varepsilon)^3$ será despreciable. Si $R_A$ es la resistencia inicial en estado austenítico, y $R$ es la resistencia en estado austenítico al retransformar, de la ecuación \ref{austret} puede despejarse $b$ correspondiente a la martensita retenida. 

%TODO calcular valores de austenita retenida.
...
% 
% 
%  Al transformar de austenita a martensita se produce un aumento en la resistencia debido a la mayor resistividad de la fase martensítica. Si en los ciclos lo único que ocurre es la transformación de fase, el aumento de resistencia debería ser siempre el mismo. Pero en cada uno de estos ciclos queda austenita retenida, lo que genera un aumento de la resistencia al volver a estado austenítico. Además, la rotura de la estructura, que ocurre por las cargas aplicadas y por las tensiones generadas en la transformación, hacen que disminuya la sección efectiva de la esponja, aumentando aún más la resistencia. 

Hasta aquí se obtuvo un parámetro que brinda información sobre la martensita retenida presente en la esponja luego de cada ciclo de transformación. Veremos ahora como obtener un parámetro con información de la integridad estructural de la esponja.  

 \begin{figure}[h]
 \centering
 \includegraphics[width=0.5\textwidth]{Img/Resultados/Resistencia/Histeresis2.eps}
...
La resistencia inicial de una esponja en estado austenítico está dada por R, figura \ref{fig:HistEsquema}. Inicialmente la esponja está compuesta de una fase austenita y no posee ninguna rotura.

\begin{equation}
 R=\rho_A \frac{L_0}{A_0}
\end{equation}

Idealmente, si se realiza un enfriamiento, la probeta transformará totalmente a martensita, y la resistencia en este punto corresponderá a la esponja en estado martensítico, $R_M$ en la figura \ref{fig:HistEsquema}. Lo que en realidad ocurre, es que en esta primera transformación, las mismas tensiones generadas por la transformación de los distintos granos, llevan a la formación de fisuras en la estructura. Por lo tanto al aumentar la resistencia por la transformación martensítica, se estará viendo también el aumento por la degradación de la estructura ($R_2$ en la figura \ref{fig:HistEsquema}). 

% El factor $f$ es cociente entre la resistencia en fase martensítica ($R_M$) y en fase austenítica ($R_A$). Por otro lado el parámetro $a$ relaciona la sección de la esponja ($A$) y la sección inicial ($A_0$).

Luego de realizar un ciclo de transformación martensítica y volver a estado austenítico, la estructura estará compuesta por austenita, martensita retenida, y fisuras en la estructura ($R_1$ en la figura \ref{fig:HistEsquema}), y estará dada por:

\begin{equation}
 R_1=\rho_A \frac{L_0}{A}(1-b) + \rho_M \frac{L_0}{A}b
\end{equation}
Donde $0<b<1$ representa la fracción de martensita retenida. Si $f$ es el cociente de la resistividad en estado martensítico y la resistividad en estado austenítico ($f=\rho_M / \rho_ A$): 
...
\end{equation}
\begin{equation}
 R_2 = \rho_M \frac{L_0}{A} = R \frac{A_0}{A} f \label{rotura}
\end{equation}

Donde $R_2$ es el valor de resistencia en estado martensítico, $R$ es el valor de resistencia en estado austenítico inicial y $R_1$ luego de retransformar a austenita. De esta forma la resistencia medida en estado martensítica $R_2$ y la resistencia en estado austenítico $R$, quedan relacionados a  un factor geométrico que permite cuantificar la rotura de la esponja $a=A_0/A$, ya que aproximamos $f=1.3$.
 
Al  aplicar la ecuación \ref{rotura} a los valores obtenidos de las curvas del gráfico \ref{fig:RED_resultados}, se obtuvieron los resultados de la tabla \ref{tab:A0Arotura}. También se evaluaron los resultados obtenidos de otras esponjas a las que se les había realizado distintos ensayos mecánicos (esponja $C$).
...
% 
% 
% Estabilizacion martensitica puede bajar la R...
% 
% Cargas hace que se toquen más puntos de la estructura y baja mucho la resistividad. Hacer ensayos luego de aplicar la carga con carga cero para poder ver las roturas separadas.



 
% encontramos el factor f cumple que: 
% 
% \begin{equation}
% \frac{1}{(1+e)^2}.f=cte
% \end{equation}
% 
% Explicar el procedimiento para llegar a los resultados.
% 
% Significa que la resistividad cambia por la forma y no por la tensión aplicada.
% 
% Relacionar con lo que queda retenido en la esponja.
% 
