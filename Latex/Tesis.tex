...
Por lo tanto, 
\begin{equation}
R= \rho \frac{l}{A} = \rho (1-\varepsilon)^2 \frac{l_0}{A_0} %= \rho f^2 \frac{l_0}{A_0}
\end{equation}
...
\vspace{50mm}
 ...
 \end{figure}
% 
% La resistencia inicial de una esponja en estado austenítico está dada por $R_A$, figura \ref{fig:HistEsquema}. Inicialmente la esponja está compuesta de una fase austenita y no posee ninguna rotura.
% 
% \begin{equation}
%  R_A=\rho_A \frac{L_0}{A_0}
% \end{equation}
% 
% Idealmente, si se realiza un enfriamiento, la probeta transformará totalmente a martensita, y la resistencia en este punto corresponderá a la esponja en estado martensítico, $R_M$ en la figura \ref{fig:HistEsquema}. Lo que en realidad ocurre, es que en esta primera transformación, las mismas tensiones generadas por la transformación de los distintos granos, llevan a la formación de fisuras en la estructura. Por lo tanto al aumentar la resistencia por la transformación martensítica, se estará viendo también el aumento por la degradación de la estructura ($R_M ^{medido}$ en la figura \ref{fig:HistEsquema}). 
% 
...
% 
% Luego de realizar un ciclo de transformación martensítica y volver a estado austenítico, la estructura estará compuesta por austenita, martensita retenida, y fisuras en la estructura (${R_A}^{medido}$ en la figura \ref{fig:HistEsquema}), y estará dada por:
% 
% \begin{equation}
%  {R_A}^{medido}=\rho_A \frac{L_0}{A}(1-b) + \rho_M \frac{L_0}{A}b
% \end{equation}
...
% 
% \begin{equation}
% {R_A}^{medido}= \rho_A \frac{L_0}{A} [(1-b)+fb]
% \end{equation}
% \begin{equation}
%  {R_A}^{medido}=R_A \frac{A_0}{A}[1+b(f-1)]
% \end{equation}
% \begin{equation}
%  R_M ^{medido} = \rho_M \frac{L_0}{A} = R_A \frac{A_0}{A} f \label{rotura}
% \end{equation}
% 
% Donde $R_M ^{medido}$ es el valor de resistencia en estado martensítico, $R_A$ es el valor de resistencia en estado austenítico inicial y $R_A ^{medido}$ luego de retransformar a austenita. De esta forma la resistencia medida en estado martensítica $R_M ^{medido}$ y la resistencia en estado austenítico $R_A$, quedan relacionados a  un factor geométrico que permite cuantificar la rotura de la esponja $a=A_0/A$, ya que aproximamos $f=1.3$.
%  
...
% 
% Si $f$ es el cociente entre las resistividades en estado martensítico y austenítico $f=\rho_M / \rho_ A$, entonces:
% \begin{equation}
% {R_A}^{medido}= \rho_A \frac{L_0}{A} [(1-b)+fb]=R_A \frac{A_0}{A}[1+b(f-1)] = R_A \rho_A \frac{L}{A} (1+\varepsilon)^2 [1-b+bf]
% \end{equation}
% \begin{equation}
%  {R_A}^{medido} = R_A (1+\varepsilon)^2 [1+b(f-1)] \label{austret}
% \end{equation}
% 
% 
% 
% De esta forma la resistencia medida en estado austenítico ${R_A}^{mdido}$ toma en cuenta un factor geométrico  $(1+ \varepsilon)^2$, y un término $b$ correspondiente a la fracción de martensita retenida. Del análisis realizado en la sección \ref{PoilicMonocrist} se encontró que $f=~1.3$ representa una buena aproximación. Además si se realizó un ciclo de transformación completo, y la probeta retransformó a austenita recuperando su forma inicial, entonces el término geométrico $(1+\varepsilon)^3$ será despreciable. Si $R_A$ es la resistencia inicial en estado austenítico, y ${R_A}^{medido}$ es la resistencia en estado austenítico al retransformar, de la ecuación \ref{austret} puede despejarse $b$ correspondiente a la martensita retenida. 
